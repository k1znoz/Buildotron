...
import { parseProjectJson, serializeProject } from '../Apps/builder/src/projectJson.ts'
import { isSafeHref } from '@buildotron/plugin-sdk'

const sample = readFileSync(new URL('../projects/product-landing.json', import.meta.url), 'utf8')
...
test('invalid sections are rejected', () => {
  ...
  assert.throws(() => parseProjectJson(JSON.stringify(project)), /sans override/)

  project.sections[1].override = true
  assert.equal(parseProjectJson(JSON.stringify(project)).sections[1].slot, 'footer')
})
...
test('unsupported versions and malformed JSON are rejected', () => {
  ...
  assert.throws(() => parseProjectJson(JSON.stringify(project)), /Version du format/)
})
...
test('legacy CTA data gains editable action defaults and preserves a configured link', () => {
  ...
  assert.equal(reopened.sections.find((section) => section.type === 'CTA').properties.actionHref, '/contact')
})

test('CTA links accept web and local destinations and reject unsafe schemes', () => {
  for (const href of ['https://example.com', 'http://example.com', '/contact', '#contact']) {
    assert.equal(isSafeHref(href), true)
  }
  for (const href of ['javascript:alert(1)', 'data:text/html,hello', '//example.com', '/\\example.com', 'https://example.com\n']) {
