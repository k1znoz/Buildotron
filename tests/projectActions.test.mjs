...
import { duplicateSection, insertionBeforeId, moveSection, moveSectionBy, setSectionOverride } from '../Apps/builder/src/projectActions.ts'
...
test('slot override is required for exceptional placement', () => {
  ...
  assert.ok(moved)
  assert.equal(moved.sections.find((section) => section.id === hero.id).slot, 'content')
  assert.deepEqual(moved.sections.filter((section) => section.slot === 'content').map((section) => section.id), [hero.id, features.id])
  const restored = setSectionOverride(moved, hero.id, false)
  assert.equal(restored.sections.find((section) => section.id === hero.id).slot, 'hero')
})
...
test('keyboard reorder only affects the selected slot', () => {
  ...
  assert.deepEqual(moved.sections.filter((section) => section.slot === 'content').map((section) => section.id), ['extra-id', features.id])
...
test('dropping onto a section moves both upward and downward', () => {
  ...
  assert.deepEqual(down.sections.filter((section) => section.slot === 'content').map((section) => section.id), ['extra-id', features.id])

  const upBefore = insertionBeforeId(down, features.id, 'extra-id', true)
  const up = moveSection(down, features.id, 'content', upBefore)
  assert.deepEqual(up.sections.filter((section) => section.slot === 'content').map((section) => section.id), [features.id, 'extra-id'])
})
