...
export function validateStepsContent(content: StepsContent): string | null {
  ...
  if (content.items.length < 1 || content.items.length > 12) {
    return 'La section doit contenir entre 1 et 12 étapes.'
  }
  if (content.items.some((item) => !item.title.trim() || !item.body.trim())) {
    return 'Chaque étape doit avoir un titre et un texte.'
  }
  return null
}
