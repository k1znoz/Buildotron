...
import { validateHeroContent } from './heroContent'
...
export function HeroAdminForm({ initialContent, onApply }: Props) {
  const [draft, setDraft] = useState<HeroContent>(initialContent)
  const [error, setError] = useState('')

  function change(key: keyof HeroContent, value: string) {
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        const issue = validateHeroContent(draft)
        if (issue) {
          setError(issue)
          return
        }
